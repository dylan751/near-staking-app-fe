...
const MyButton = (props: { loading: boolean, disable: boolean, text: string, onClick?: Function }) => {
    ...
    return (
      <div className={style} onClick={(event) => props.onClick()}>
          <button className={disable ? "w-full h-full cursor-not-allowed" : "w-full h-full cursor-pointer"}>
              {buttonText}
              {
                  props.loading &&
                  <Spin className={"ml-2"} indicator={<LoadingOutlined className={"text-white"} style={{ fontSize: 24 }} spin />} />
              }
          </button>
      </div>
    )
